...
#pragma HLS pipeline
          ...
          // ix[r][c] =
          //    img[r - 1][c - 1] * (-1 / 12.0) + img[r + 1][c - 1] * (1 / 12.0)
          //    + img[r - 1][c] * (-2 / 12.0) + img[r + 1][c] * (2 / 12.0) +
          //    img[r - 1][c + 1] * (-1 / 12.0) + img[r + 1][c + 1] * (1
          //    / 12.0);
        }
      }
      ix[r][c] = acc;
...
#pragma HLS pipeline
          ...
          // iy[r][c]= img[r - 1][c - 1] * (-1 / 12.0) +
          //       img[r - 1][c + 1] * (1 / 12.0) + img[r][c - 1] * (-2 / 12.0)
          //       + img[r][c + 1] * (2 / 12.0) + img[r + 1][c - 1] * (-1
          //       / 12.0) + img[r + 1][c + 1] * (1 / 12.0);
        }
      }
      iy[r][c] = acc;
    }
...
#pragma HLS INTERFACE mode = ap_memory port = harris storage_type = ram_1p
  ...
  float img1[R][C];
  //#pragma HLS ARRAY_PARTITION dim = 1 factor = 1 type = cyclic variable = img1
  //#pragma HLS bind_storage variable = img1 impl = bram type = ram_s2p
  float img2[R][C];
  //#pragma HLS ARRAY_PARTITION dim = 1 factor = 1 type = cyclic variable = img2
  ...
  float ixx[R][C];
  //#pragma HLS ARRAY_PARTITION dim = 1 factor = 1 type = cyclic variable = ixx
  //#pragma HLS bind_storage variable = ixx impl = bram type = ram_s2p
  float iyy[R][C];
  //#pragma HLS ARRAY_PARTITION dim = 1 factor = 1 type = cyclic variable = iyy
  //#pragma HLS bind_storage variable = iyy impl = bram type = ram_s2p
  float ixy[R][C];
  //#pragma HLS ARRAY_PARTITION dim = 1 factor = 1 type = cyclic variable = ixy
